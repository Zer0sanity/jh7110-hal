...
impl Arg {
    ...
    /// Gets the [MaskWrite] for the `CMD49` [Arg].
    pub const fn mask_write(&self) -> Result<MaskWrite> {
        MaskWrite::from_raw((self.0 & 0x400_0000) >> 26)
    }

    /// Gets the [MaskWrite] for the `CMD49` [Arg].
    pub fn set_mask_write(&mut self, val: MaskWrite) {
        self.0 = (self.0 & !0x400_0000) | (val.into_raw() << 26);
    }

    /// Gets the [MIO] for the `CMD49` [Arg].
...
mod tests {
    ...
    fn test_fields() {
        ...
        [MaskWrite::Disable, MaskWrite::Enable]
            .into_iter()
            .for_each(|exp_mask| {
                arg.set_mask_write(exp_mask);

                assert_eq!(arg.mask_write(), Ok(exp_mask));
                assert_eq!(arg.bits(), exp_mask.into_raw() << 26);
            });

        arg.set_mask_write(MaskWrite::Disable);
